...
The dashboard is a data visualizer integrated into net, intended to collect and visualize useful information of an Blockchain node. It consists of two parts:
...
Normally the dashboard assets are bundled into Lov via `go-bindata` to avoid external dependencies. Rebuilding Lov after each UI modification however is not feasible from a developer perspective. Instead, we can run `yarn dev` to watch for file system changes and refresh the browser automatically.

```
$ lov --dashboard --vmodule=dashboard=5
$ (cd dashboard/assets && yarn dev)
```

To bundle up the final UI into Lov, run `go generate`:
