...
func TestAccountListEmpty(t *testing.T) {
	lov := runLov(t, "account", "list")
	lov.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	lov := runLov(t, "account", "list", "--datadir", datadir)
	defer lov.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	lov := runLov(t, "account", "new", "--lightkdf")
	defer lov.ExpectExit()
	lov.Expect(`
...
`)
	lov.ExpectRegexp(`Address: \{[0-9a-f]{40}\}\n`)
}
...
func TestWalletImport(t *testing.T) {
	lov := runLov(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer lov.ExpectExit()
	lov.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(lov.Datadir, "keylovre"))
	if len(files) != 1 {
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(lov.StderrText(), m) {
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	lov.SetTemplateFunc("keypath", func(file string) string {
