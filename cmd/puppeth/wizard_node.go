...
func (w *wizard) deployNode(boot bool) {
	...
	if err != nil {
		if boot {
			infos = &nodeInfos{port: 44446, peersTotal: 512, peersLight: 256}
		} else {
			infos = &nodeInfos{port: 44446, peersTotal: 50, peersLight: 0, gasTarget: 4.7, gasPrice: 18}
		}
	}
	existed := err == nil
	...
	if !boot {
		if w.conf.Genesis.Config.Ethash != nil {
			// Ethash based miners only need an loverbase to mine against
			fmt.Println()
			if infos.loverbase == "" {
				fmt.Printf("What address should the miner use?\n")
				for {
					if address := w.readAddress(); address != nil {
						infos.loverbase = address.Hex()
						break
					}
				}
			} else {
				fmt.Printf("What address should the miner use? (default = %s)\n", infos.loverbase)
				infos.loverbase = w.readDefaultAddress(common.HexToAddress(infos.loverbase)).Hex()
			}
