Clef
----
Clef can be used to sign transactions and data and is meant as a replacement for net's account management.
This allows DApps not to depend on net's account management. When a DApp wants to sign data it can send the data to
...
The general flow for signing a transaction using e.g. lov is as follows:
![image](sign_flow.png)

In this case, `lov` would be started with `--externalsigner=http://localhost:8550` and would relay requests to `lov.sendTransaction`.
...
put together is a bit of a hack into the http server. This could probably be greatly improved
    - Relay: Lov should be started in `lov --external_signer localhost:8550`.
    - Currently, the Lov APIs use `common.Address` in the arguments to transaction submission (e.g `to` field). This
  ...
  retains the original input.
    - The Lov api should switch to use the same type, and relay `to`-account verbatim to the external api.
...
* Another potential thing to introduce is pairing.
  ...
  * Thus lov/mist/cpp would cryptographically handshake and afterwards the caller would be allowed to make signing requests.
...
The signer listens to HTTP requests on `rpcaddr`:`rpcport`, with the same JSONRPC standard as Lov. The messages are
