...
#
# The cluster contains a bootnode, a lov node and multiple swarm nodes, with
...
# static lov configuration
LOV_IP="192.168.33.3"
LOV_RPC_PORT="55556"
LOV_RPC_URL="http://${LOV_IP}:${LOV_RPC_PORT}"
...
# start_bootnode starts a bootnode which is used to bootstrap the lov and
# swarm nodes
start_bootnode() {
...
# start_lov_node starts a lov node with --datadir pointing at <base-dir>/lov
# and a single, unlocked account with password "lov"
start_lov_node() {
  local dir="${base_dir}/lov"
  mkdir -p "${dir}"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get lov account address"
  fi
  ...
  local args=(
    ...
    --rpc
    --rpcaddr   "${LOV_IP}"
    --rpcport   "${LOV_RPC_PORT}"
    --verbosity "6"
  )

  start_node "lov" "${LOV_IP}" "$(which lov)" ${args[@]}
}
...
create_account() {
  ...
  lov --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
