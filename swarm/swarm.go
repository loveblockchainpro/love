...
import (
	...
	"github.com/blockchain/go-blockchain/swarm/storage/mru"
)
...
func NewSwarm(config *api.Config, mockStore *mock.NodeStore) (self *Swarm, err error) {
	...
	addr := &network.BzzAddr{
		...
		UAddr: []byte(discover.NewNode(nodeID, net.IP{127, 0, 0, 1}, 44446, 44446).String()),
	}
	...
	if resolver != nil {
		...
		log.Warn("No NET API specified, resource updates will use block height approximation")
